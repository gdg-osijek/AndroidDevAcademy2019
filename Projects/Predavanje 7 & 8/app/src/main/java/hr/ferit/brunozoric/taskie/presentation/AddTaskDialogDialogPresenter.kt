...
class AddTaskDialogDialogPresenter(private val interactor: TaskieInteractor) : AddTaskDilogContract.Presenter {
    ...
    override fun onAddTask(task: AddTaskRequest) {
        if (task.content.isNotBlank() && task.title.isNotBlank()) {
    ...
    private fun addTaskCallback(): Callback<BackendTask> = object : Callback<BackendTask> {
        ...
        override fun onResponse(call: Call<BackendTask>?, response: Response<BackendTask>) {
            onAddTaskResponse(response)
        }
    }

    internal fun onAddTaskResponse(response: Response<BackendTask>) {
        if (response.isSuccessful) {
            onAddTaskResponseCode(response.code(), response.body())
        } else {
            view.showDataError()
        }
    }

    internal fun onAddTaskResponseCode(code: Int, body: BackendTask?) {
        when (code) {
            RESPONSE_OK -> handleOkResponse(body)
            else -> handleSomethingWentWrong()
        }
    }

    internal fun handleOkResponse(task: BackendTask?) = task?.run { view.onTaskAdded(this) }

    internal fun handleSomethingWentWrong() {
